...
**当前版本:[v0.1.1](https://github.com/Lenssansi/ai-helper/releases)** —— 正在重构为
AstrBot 本地部署 + ai-helper 独特功能(VPN/Provider/编程 Agent/组件管理等)插件化。
**v0.0.5 是最后一个独立架构版本**;v0.1.x 完成前桌面安装包暂不更新。
后续版本变动同步更新本节。
...
| v0.0.4 | 安全加固(CSRF 防护 + 远程权限收紧 + VPN/SSRF + 常量时间比较)+ VPN 内核按需获取(惰性提示 + SHA256 校验)+ 设置页「组件与更新」统一面板 + 滚动条补全 | ✅ 完成 |
| v0.0.5 | 修文件模式停止卡死(Popen+kill,新增 /api/chatfs/stop)+ 工具调用进行中转圈提示 + 大脑后端可换云端(便宜 Flash 类 ¥1-2/月,效果优于本地)+ 综合测试按钮 + 联网搜索改 Tavily(独立配置,SettingsPage 单独段) | ✅ 完成 |
| **v0.1.x 重构** | 改用 AstrBot 为底座(Electron 壳 + 本地 SQLite),ai-helper 独特功能(VPN/mihomo/Provider 余量/编程 Agent + Git/组件管理/grill-with-docs)写成 AstrBot 插件 | 🚧 进行中 |
